using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class Enemyleft : MonoBehaviour
{
	public float SpeedMove;
	private Rigidbody2D rb;
	[SerializeField]
	private GameObject bullet;

	public Vector3 move;

	float minY;
	float maxY;

	float minX;
	float maxX;

	float y;
	float x;

	float t = 0;
	public Transform splineRoot;
	//========================
	void Start()
	{
		Vector3 bounds = Camera.main.ScreenToWorldPoint(new Vector3(Screen.width, Screen.height, 0f));

		maxY = bounds.y - Random.Range(1f, 4f);
		maxX = bounds.x -Random.Range(1f,4f);
		Debug.Log(maxX + " max X");
		rb = GetComponent<Rigidbody2D>();
		InvokeRepeating("Shoot", 1f, 1f);

		y = bounds.y;

	}
	//============= Update
	void Update()
	{
		EnemyMove();
		
	}
	
	//======================
	void EnemyMove()
	{

		if (transform.position.y <= maxY && transform.position.x >= maxX)
		{
			
			transform.position = new Vector3(transform.position.x, transform.position.y, 0f);
		}
		else
		{
			y -= SpeedMove * Time.deltaTime;
			x += SpeedMove * Time.deltaTime;

			t += Time.deltaTime * SpeedMove;
			transform.position = splineRoot.position + splineRoot.forward * Mathf.Sin(t) * 2f;
		}
	}

	//=====================

	void Shoot()
	{
		Vector3 temp = transform.position;
		temp.y -= 1.5f;
		Instantiate(bullet, temp, Quaternion.identity);

	}
	//===================
}
